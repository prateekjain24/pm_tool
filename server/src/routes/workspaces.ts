...
workspaceRouter.get("/", async (c) => {
  ...
  const userWorkspaces = Array.from(mockWorkspaces.values()).filter(ws => 
    ws.ownerId === user.id || ws.members.some(m => m.userId === user.id)
  );
...
workspaceRouter.get("/:id", async (c) => {
  ...
  const hasAccess = workspace.ownerId === user.id || 
...
workspaceRouter.put(
  ...
  async (c) => {
    ...
    const hasAdminAccess = workspace.ownerId === user.id || 
